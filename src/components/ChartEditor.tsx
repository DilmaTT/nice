...
export const ChartEditor = ({ isMobileMode = false, chart, onBackToCharts, onSaveChart }: ChartEditorProps) => {
  ...
  // State for the new folder/range selects in the modal
  ...
  // Effect to set the selected folder when the modal opens
  useEffect(() => {
  ...
  // Derived state for the ranges in the selected folder
  ...
  const handleAddButton = () => {
    const newButton: ChartButton = {
      ...
      name: "New",
      color: "#60A5FA",
      linkedItem: "label-only", // Default to label
      x: 50,
      y: 50,
        ...
        <Dialog open={isButtonModalOpen} onOpenChange={(isOpen) => {
          ...
          <DialogContent mobileFullscreen={isMobileMode}>
            ...
            <div className="grid gap-4 py-4">
              ...
              {/* Conditional Folder/Range Selects */}
